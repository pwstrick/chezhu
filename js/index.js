 ...
 */
var template = {
    img_str: function(sort) {
        return 'img/coupon/'+sort+'.jpg';
    },
    coupon: function() {//选择的优惠券列表
        var _this = this;
        var json = {
            "code": 0,
            "msg": "success",
        ...
        var html = Mustache.render($('#template').html(), {
            data: chelunData, name_str: function () {
        ...
        html = Mustache.render($('#template2').html(), {
            data: otherData, name_str: function () {
                return this.name.substr(0, 4);
            }, css: function () {
                if (this.num == 1)
                    return '';
    ...
    welfare: function() {//选中的优惠券列表
        var _this = this;
        var coupon = env.welfare();
        var html = Mustache.render($('#template').html(), {data:coupon.coupon, img_str: function() {
            return _this.img_str(this.sort);
        }});
    ...
    result: function() {//优惠券结果
        var _this = this;
        var coupon = env.welfare();
        var html = Mustache.render($('#template').html(), {data:coupon.coupon, name_str: function() {
            return this.name.substr(0, 4);
        }, img_str: function() {
            return _this.img_str(this.sort);
        }});
...
if($txtMobile.length > 0) {
    ...
    $('#btnPay').on('click', function() {
        ...
        //优惠券已领过券
        Alert('此号码已领取过优惠券', {confirmFn:function() {
